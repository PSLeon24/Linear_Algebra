...
- 벡터(vector, <b>v</b>): 순서가 정해져 있는 일종의 배열(array), 일반적으로 열벡터로 나타내고 굵은 소문자로 나타냄 ↔ 순서가 정해져 있지 않은 것은 집합(set)
  ...
  - 벡터의 차원(dimensionality): 벡터가 가진 원소의 수
    - e. g., R²: 두 개의 원소가 있는 벡터
      - <b>x</b> = [x1, x2] 또는 <b>x</b> = [x1, x2]ᵀ
    - e. g., R³: 세 개의 원소가 있는 벡터
      - <b>x</b> = [x1, x2, x3] 또는 <b>x</b> = [x1, x2, x3]ᵀ
    - R⁰: 영차원 부분공간 (차원 = 0)
    - Rⁿ: n차원 공간 (차원 = n)
  ...
  - 영벡터(zero vector): 모든 원소가 0인 벡터, denoted by <b>0</b>
  - Practice
    <pre>
...
- 행렬의 표기법
  ...
  - Aᵀ: 벡터 또는 행렬의 전치, 주대각 성분들을 기준으로 행렬을 회전 ~ 즉, 행과 열을 교환하는 것(a(i,j)ᵀ = a(j,i))
    - 벡터의 경우, 열벡터를 행벡터로 변환하거나 행벡터를 열벡터로 변환함
    - 중요! 벡터 또는 행렬을 이중 전치하면 원래 벡터 또는 행렬이 됨. (Aᵀ)ᵀ = A
      - 당연한 것 같지만 AI에서 중요한 증명에서 핵심 근거가 됨.
      - LIVE EVIL(연산 순서)
        - (LIVE) = EᵀVᵀIᵀLᵀ
          - 여러 행렬의 곱셈을 전치할 때의 순서, 위와 같이 4개뿐만 아니라 더 많은 수의 행렬에도 동일하게 위의 규칙이 적용됨
    - ![image](https://github.com/PSLeon24/Linear_Algebra/assets/59058869/6317db12-c84a-4248-90c2-2bdedd1b0114)
    - ![image](https://github.com/PSLeon24/Linear_Algebra/assets/59058869/c2fe1483-0fd9-4a2a-aee0-284bef4708b2)
    - ![image](https://github.com/PSLeon24/Linear_Algebra/assets/59058869/73e3d9fe-d621-4100-b70a-7bcf90286be8)
    - ![image](https://github.com/PSLeon24/Linear_Algebra/assets/59058869/b700aba3-f69a-413c-9c88-6f082651a628)
    - ![image](https://github.com/PSLeon24/Linear_Algebra/assets/59058869/ba160091-85be-4d41-a7ab-e42ff2d119af)
...
- 특수 행렬
  ...
  - 대칭 행렬
    - 자신의 전치 행렬과 동일한 행렬(A = Aᵀ, 정방행렬일 때만 가능)
      - 수치적으로 안정적인 경향이 있어서 컴퓨터 알고리즘을 처리할 때 편리하고 여러 작업에 편리학 특성이 많음
    - ![image](https://github.com/PSLeon24/Linear_Algebra/assets/59058869/f083c302-36d4-4a1d-b66f-8b31f4d0603f)
...
- 행렬 노름(||A||)
  ...
  - 원소별 노름: 행렬의 개별 원소를 기반으로 계산되는 노름 → 행렬의 원소의 크기를 반영해서 해석
    - 프로베니우스 노름(Frobenius Norm, 유클리드 노름, L2 norm)
      ...
      - ![image](https://github.com/PSLeon24/Linear_Algebra/assets/59058869/1d714be2-116d-4e3d-a47e-a53ed73074fd)
      ...
      - 프로베니우스 노름은 어떤 행렬의 전치와 그 행렬을 곱한 결과의 대각합의 제곱근으로 계산할 수 있음
        - ![image](https://github.com/PSLeon24/Linear_Algebra/assets/59058869/41e68d87-2c2f-4793-a116-c5d9e4f93315)

          - 성립 이유: AᵀA의 각 대각 원소는 동일한 행에 대한 내적이기 때문
...
- 행렬 공간
  ...
  - 열공간(column space)
    ...
    - 열공간의 차원 수는 열들이 선형 독립 집합을 형성하는 경우에만 열수와 동일
      - 집합 내의 어떤 벡터도 해당 집합의 다른 벡터들의 선형 결합으로 표현될 수 없을 때 linear independent 하다.
  - 행공간(row space)
...
- 행렬 노름에는 많은 종류가 있는데 <b>원소별</b>과 <b>유도별</b>로 크게 분류할 수 있음
...
## Theorem
1.1.1. <b>x, y, z</b>가 Rⁿ의 벡터이고 h와 k가 스칼라일 때,
  - (1) <b>x + y = y + x</b>
  - (2) <b>(x + y) + z = x + (y + z)</b>
  - (3) <b>x + 0 = x = 0 + x</b>
  - (4) <b>x + (-x) = 0 = (-x) + x</b>
  - (5) k(<b>x + y</b>) = k<b>x</b> + k<b>y</b>
  - (6) (h + k)<b>x</b> = h<b>x</b> + k<b>x</b>
  - (7) (hk)<b>x</b> = h(k<b>x</b>)
  - (8) 1<b>x</b> = <b>x</b>
  
1.1.2 <b>x</b>가 Rⁿ의 벡터이고 k가 스칼라일 때,
  - (1) 0<b>x</b> = <b>0</b>
  - (2) k<b>0</b> = <b>0</b>
  - (3) (-1)<b>x</b> = -<b>x</b>
